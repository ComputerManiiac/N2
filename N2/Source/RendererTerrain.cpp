...
RendererTerrain::RendererTerrain(ShaderProgram* shader) : Renderer(shader) {}
...
void RendererTerrain::Initialize(const BatchKey& key, Batch& batch)
{
	...
	shader->setUniform("terrainBackground", 2);
	shader->setUniform("terrainMountain", 3);
	shader->setUniform("terrainMud", 4);
	shader->setUniform("terrainPath", 5);
...
void RendererTerrain::Render(Batch& batch, const unsigned int& textureID, MS& modelStack, const Mtx44& view)
{
	shader->Use();
...
void RendererTerrain::Deinitialize(Batch& batch)
{
